...
var algos = module.exports = global.algos = {
    ...
    yescrypt: {
        ...
        hash: function(coinconfig){
            var n = 2048;
            var r = 8;
            var client_key_hack = true;

            if (coinconfig.yescryptParam) {
                var param = coinconfig.yescryptParam;
                if (param === 'yescrypt' || param === 'yescrypt8') {
                    n = 2048;
                    r = 8;
                    client_key_hack = true;
                }
                else if (param === 'yescryptr8') {
                    n = 2048;
                    r = 8;
                    client_key_hack = false;
                }
                else if (param === 'yescryptr16') {
                    n = 4096;
                    r = 16;
                    client_key_hack = false;
                }
                    ...
                    n = param.n;
                    r = param.r;
                    client_key_hack = param.client_key_hack;
                }
                else {
                    throw new Error('Invalid yescryptParam.');
                }
            }

            return function(data){
                return multiHashing.yescrypt(data, n, r, client_key_hack);
            }
        }
    }
};
